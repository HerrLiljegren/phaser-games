...
Machine.Main.prototype = {
    ...
    update: function() {
        ...
        this.game.physics.arcade.collide(this.player.weaponCannons.bullets, Machine.LevelManager.layer, function(bullet, tile) {
            ...
            if(tile.faceTop) { impactPoint.x = bullet.position.x; impactPoint.y = tile.worldY; phaserDirection = Phaser.UP; }
            if(tile.faceBottom) { impactPoint.x = bullet.position.x; impactPoint.y = tile.worldY+tile.height; phaserDirection = Phaser.DOWN }
            if(tile.faceRight) { impactPoint.x = tile.worldX + tile.width; impactPoint.y = bullet.position.y; phaserDirection = Phaser.RIGHT }
            if(tile.faceLeft) { impactPoint.x = tile.worldX; impactPoint.y = bullet.position.y; phaserDirection = Phaser.LEFT }
